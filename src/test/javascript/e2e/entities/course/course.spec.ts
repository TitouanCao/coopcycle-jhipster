...
describe('Course e2e test', () => {
  ...
  it('should create and save Courses', async () => {
    ...
    await promise.all([
      courseUpdatePage.setTimeRequiredInput('5'),
      courseUpdatePage.orderSelectLastOption(),
      // courseUpdatePage.agentsSelectLastOption(),
      courseUpdatePage.restaurantSelectLastOption(),
    ]);

    expect(await courseUpdatePage.getTimeRequiredInput()).to.eq('5', 'Expected timeRequired value to be equals to 5');
